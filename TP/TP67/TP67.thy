...
*)
fun addVarDangerous :: "string  \<Rightarrow>  tabSymbDangerous \<Rightarrow>  tabSymbDangerous"
where
"addVarDangerous v [] = (v#[])" |
"addVarDangerous v (hv#tv) = (if v=hv 
                                 then (hv#tv) 
                                 else (hv#(addVarDangerous v tv)))"
...
*)
fun addVar :: "string  \<Rightarrow> int \<Rightarrow> tabSymb \<Rightarrow> tabSymb"
where
"addVar v i [] = ((v, i)#[])" |
"addVar v i ((hv, hi)#tx) = (if v=hv 
                                 then ((hv, i)#tx) 
                                 else ((hv, hi)#(addVar v i tx)))"
...
*)
fun processReadV :: "string \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list"
where
"(processReadV v []) = []"|
"(processReadV v ((hbool, htSD, htS)#tx)) = ((hbool, (addVarDangerous v htSD), (rmVar v htS)) # (processReadV v tx))"


(* processAffVE
RECUPERE: la variable a modifier ET l'expression de remplacement ET la list des tests,tables des symboles (cas a tester)
RETOURNE: rend la list des tests,tables des symboles a jour (pour un aff)
*)
fun processAffVE :: "string \<Rightarrow> expression \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list"
where
"processAffVE v e [] = []"|
"processAffVE v e ((hbool, htSD, htS)#tx) = (if (containDangerousVar e htSD htS)
                                             then ((hbool, (addVarDangerous v htSD), (rmVar v htS))#(processAffVE v e tx))
                                             else ((hbool, (rmVarDangerous v htSD), (addVar v (evalExp e htS) htS))#(processAffVE v e tx)))"


(* isThenAccessibleUnitaire
RECUPERE: la condition ET un element de la liste des tests,tables des symboles (cas a tester)
RETOURNE: vrai si le then est accessible (cond possiblement valable), faux sinon
*)
fun isThenAccessibleUnitaire :: "( condition \<Rightarrow> (bool * tabSymbDangerous * tabSymb)  \<Rightarrow> bool)"
where
"(isThenAccessibleUnitaire (Eq e1 e2) (_, tSD, tS) ) = (if ((containDangerousVar e1 tSD tS) \<or> (containDangerousVar e2 tSD tS) )
                                                then True
                                                else ((evalExp e1 tS) = (evalExp e2 tS)))"

(* isThenAccessible
RECUPERE: la condition ET la list des tests,tables des symboles (cas a tester)
RETOURNE: vrai si le then est accessible (cond possiblement valable), faux sinon
*)
fun isThenAccessible :: "( condition \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list \<Rightarrow> bool)"
where
"(isThenAccessible e []) = False"|
"(isThenAccessible e (hList#tList)) = ((isThenAccessibleUnitaire e hList) \<or> (isThenAccessible e tList))"
  
(* isElseAccessibleUnitaire
RECUPERE: la condition ET un element de la liste des tests,tables des symboles (cas a tester)
RETOURNE: vrai si le else est accessible (cond possiblement non valable), faux sinon
*)
fun isElseAccessibleUnitaire :: "( condition \<Rightarrow> (bool * tabSymbDangerous * tabSymb)  \<Rightarrow> bool)"
where
"(isElseAccessibleUnitaire (Eq e1 e2) (_, tSD, tS) ) = (if ((containDangerousVar e1 tSD tS) \<or> (containDangerousVar e2 tSD tS) )
                                                then True
                                                else \<not>((evalExp e1 tS) = (evalExp e2 tS)))"

(* isElseAccessible
RECUPERE: la condition ET la list des tests,tables des symboles (cas a tester)
RETOURNE: vrai si le else est accessible (cond possiblement non valable), faux sinon
*)
fun isElseAccessible :: "( condition \<Rightarrow> (bool * tabSymbDangerous * tabSymb) list \<Rightarrow> bool)"
where
"(isElseAccessible e []) = False"|
"(isElseAccessible e (hList#tList)) = ((isElseAccessibleUnitaire e hList) \<or> (isElseAccessible e tList))"
...
where
"(san3rec (Exec e) listTests) = (isExpSafeList e listTests)"|
"(san3rec (Read v) listTests) = (processReadV v listTests)"|
"(san3rec (Aff v e) listTests) = (processAffVE v e listTests)"|
"(san3rec (Seq s1 s2) listTests) = (san3rec s2 (san3rec s1 listTests))"|
"(san3rec (If cond s1 s2) listTests) = (if (isThenAccessible cond listTests)
                                     then
                                        (if (isElseAccessible cond listTests)
                                         then((san3rec s1 listTests)@(san3rec s2 listTests)) 
                                         else (san3rec s1 listTests)
                                        )
                                     else (san3rec s2 listTests)
                                    )"|
"(san3rec _ listTests) = listTests"

fun san3 :: "statement \<Rightarrow> bool"
where
"san3 st = (\<not>(isOneFalse (san3rec st ((True,[] ,[] )#[]) ) ))"
...
(******)
"san3rec (If cond s1 s2) tSD tS = (if (isThenAccessible cond testsList)
